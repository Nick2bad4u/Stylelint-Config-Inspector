...
describe('resolvePayload', () => {
  ...
  it('sets configs open state to expanded when there are fewer than ten configs', () => {
    ...
    expect(configsOpenStateMock.value).toEqual([true, true, true])
  })
  ...
  it('builds extends info and glob maps from config files and ignores', () => {
    ...
    expect(resolved.extendsInfoMap.get('stylelint-config-recommended')?.source).toBe('package')
    expect(resolved.globToConfigs.get('src/**/*.css')?.map(config => config.index)).toEqual([0, 1])
    expect(resolved.globToConfigs.get('dist/**')?.map(config => config.index)).toEqual([0])
    expect(resolved.globToConfigs.get('packages/**/*.scss')?.map(config => config.index)).toEqual([1])
  })
  ...
  it('creates matched, declared, and default file groups from payload files and config globs', () => {
    ...
    if (!filesResolved)
      throw new Error('Expected filesResolved data')
    ...
    expect(groupsById.get('globs:<general>')?.files).toEqual(['src/general.css'])

    expect(groupsById.get('declared-glob:src/unused/**/*.css')?.kind).toBe('declared')
    expect(groupsById.get('declared-glob:packages/**/*.scss')?.kind).toBe('declared')

    const defaultGroup = groupsById.get(`default-scan:${DEFAULT_WORKSPACE_SCAN_GLOBS[0]}`)
    expect(defaultGroup?.kind).toBe('default')
    expect(defaultGroup?.configs.map(config => config.index)).toEqual([0])
  ...
  it('does not create declared/default groups when those globs already have matched files', () => {
    ...
    const groupsById = new Map(
      (resolved.filesResolved?.groups ?? []).map(group => [group.id, group] as const),
    )

    expect(groupsById.has('declared-glob:src/**/*.css')).toBe(false)
    expect(groupsById.has(`default-scan:${matchedDefaultGlob}`)).toBe(false)
  })
})
